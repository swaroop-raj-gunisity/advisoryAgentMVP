...
*   `advisor`: Stores advisor profile, role, employee ID, department, allocated book, AUM managed, and employee manager.
*   `contact`: Stores contact name, phone, marital status, birthdate.
*   `account`: Stores total AUM ($1.8M), investment objective (Growth & Income), risk profile (Moderate), and advisor ID.
...
### Seed Validation Verification
The database integrity is verified by querying the tables directly:
```
--- ADVISOR DETAILS ---
('ADV_001', 'Sarah', 'Mitchell', 'Senior Wealth Advisor & Practice Lead', 'EMP_9021', 'Private Wealth Management', 'Book of Business A', 180000000.0, 'David Harrison, Head of Private Wealth')

--- ACCOUNTS ---
('ACC_001', 'Chen Household', 1800000.0, 'ADV_001')
```
