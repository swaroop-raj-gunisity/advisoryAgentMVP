...
### Primary Persona: Sarah Mitchell, CFP® (Advisory Persona)
*   **Role:** Senior Wealth Advisor & Practice Lead
*   **Employee ID:** EMP_9021
*   **Department:** Private Wealth Management
*   **Employee Manager:** David Harrison (Head of Private Wealth)
*   **Client Book Allocated:** Book of Business A (130 Households)
*   **AUM Managed:** $180,000,000.00
...
## 11. Setup /evals leveraging DeepEval 

High Level Pointers ( Need  detailed instructions)

Gather Agentic interations sampling data, for passing though the test scenarios ( Iitial scoring)
Identify key metrics type required for MVP based on each features 

Context for metrics     
Here's a **one-line summary for each metric** based on the detailed documentation:

| # | Metric | One-Line Summary |
|---|--------|------------------|
| 1 | **Faithfulness** | Verifies that LLM claims are grounded in retrieved context and not hallucinated. |
| 2 | **Answer Relevancy** | Ensures the chatbot's response directly addresses the user's specific question. |
| 3 | **Contextual Precision** | Measures if relevant information is ranked higher than irrelevant content in retrieval results. |
| 4 | **Contextual Recall** | Verifies that retrieval found all necessary information needed to answer a query completely. |
| 5 | **Contextual Relevancy** | Calculates the signal-to-noise ratio of retrieved content to filter out distracting irrelevant data. |
| 6 | **Hallucination** | Detects logical inconsistencies and fabricated details that contradict provided context. |
| 7 | **Bias** | Ensures output is free from gender, racial, political, and religious stereotypes or discrimination. |
| 8 | **Toxicity** | Filters offensive, harmful, hateful, or threatening language from AI responses. |
| 9 | **Summarization** | Evaluates if key facts are preserved when condensing long documents without introducing new hallucinations. |
| 10 | **G-Eval (Custom)** | Allows custom business logic evaluation with LLM-as-a-judge following your specific criteria and chain-of-thought. |
| 11 | **Task Completion** | Determines if an AI Agent successfully achieved the user's intended multi-step goal. |
| 12 | **Tool Correctness** | Verifies that an Agent selected the correct APIs with correct arguments for the given task. |
| 13 | **Conversation Completeness** | Assesses if a multi-turn conversation resolved the user's intent or left them unresolved. |
