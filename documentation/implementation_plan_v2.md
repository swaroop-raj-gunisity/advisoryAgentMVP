...
3. **Simulated Data Sources:**
   * Local SQLite Database (`advisor_portal.db`) populated with Advisor attributes, Salesforce FSC contacts, relationship notes, tasks, life events, and custodian holding details.
...
* **DB Verification Command:** Open the SQLite database and query the `advisor` table to verify Sarah Mitchell's profile exists. Query holdings to verify that the Chen Household ($1.8M AUM) contains exactly 10 holding records (1 Active Bond Fund, 8 ETFs, and 1 Cash balance) linked to advisor `ADV_001`.
